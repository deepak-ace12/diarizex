...
def get_chunk_transcription(request_id, audio_path, model_key, chunk_duration, audio_file_key):
...
def load_model(beam_size, temperature, no_speech_threshold, model_size):
    load_url = f"{base_url}/load_model"
    params = {
        "beam_size": beam_size,
    ...
    init_url = f"{load_url}?{'&'.join(f'{key}={value}' for key, value in params.items())}"
        ...
        for beam_size in beam_sizes:
            for nst in nsts:
                try:
                    ...
                    model_key = load_model(beam_size=beam_size, temperature=temperature, no_speech_threshold=nst, model_size=model_size)
                    ...
                    for idx, audio in enumerate(audios):
                        ...
                        chunk_transcript = get_chunk_transcription(request_id=request_id, audio_path=audio, model_key=model_key, chunk_duration=chunk_duration, audio_file_key=audio_file_key)
                        ...
                        file_dir2 = f"{permformance_output_base_path}/chunk_performance_{mapping.get(model_size)}/{config_key}"
                ...
                delete_url = f"{base_url}/model/{model_key}?request_id={request_id}"
                response = requests.delete(delete_url)
                print(response.json())
