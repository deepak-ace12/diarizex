...
model = whisperx.load_model("base", device, compute_type=compute_type, asr_options={"beam_size":1})
...
def transcribe(audio_file, diarize=False):
    ...
    def transcribe_audio():
        ...
        result = whisperx.align(transcript["segments"], model_a, metadata, audio, device, return_char_alignments=False)
            ...
            for word in word_segments[index:]:
                ...
                if word.get("word"):
                    if not word.get("start") or not word.get("end"):
                        ...
                        continue
                    if word.get("start") >= segment.start and word.get("end") <= segment.end:
                        sentence += " " + word.get("word")
                    else:
                        index -= 1
                        break
    ...
    try:
        if diarize:
            ...
            diarize_segments, transcribed_segments = dask.compute(delay_diarize, delay_transcribe)
            speaker_segments = assign_word_speakers(diarize_segments, transcribed_segments)
            ...
            transcript = " ".join([segment.get("text") for segment in transcribed_segments.get("segments", [])])
...
audio_files = []
for i in range(113):
    audio_files.append(f"/Users/I1597/Downloads/amit_v/{i}.wav")
...
for audio_file in audio_files:
    ...
    for idx, segment in enumerate(transcript_segments):
        ...
        if current_speaker and current_speaker in combined_speaker_data:
            if (current_end-current_start) > (combined_speaker_data.get(current_speaker, {}).get("end", 0) - combined_speaker_data.get(current_speaker, {}).get("start", 0)):
